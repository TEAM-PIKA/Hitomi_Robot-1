...
def new_fed(update, context):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @hitomi_support if the problem persist."
            )
            return
...
def join_fed(update, context):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @Hitomi_Support should this problem persist!"
            )
            return
...
def fed_ban(update, context):
    ...
    if not x:
        message.reply_text(
            "Failed to ban from the federation! If this problem continues, contact @Hitomi_Support."
        )
        return
...
def set_frules(update, context):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @Hitomi_Support !"
            )
            return
